...
<div class="nav2">
  <div class="container">
    ...
    @foreach(config('navigation.main') as $name => $data)
      <a href="{{ $data['url'] }}" class="nav-link d-none d-md-inline-block {{ $data['color'] }}">
        <i class="{{ $data['icon'] }}"></i>
        {{ __($name) }}
      </a>
    @endforeach
    <div href="#more" class="nav-link d-none d-md-inline-block" @click="showmoreclick = !showmoreclick" >
      {{ __('More') }}
