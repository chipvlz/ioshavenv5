...
<div class="container mb-4">
  <h1 class="py-2 border-bottom">{{$role->name}} Permissions</h1>
</div>
  ...
  <div class="container d-flex flex-wrap">
    ...
    <table class="table table-hover table-light">
      ...
      <tbody class="">
        ...
        <tr>
          <th scope="row" class="align-middle">{{ $perm->id }}</th>
          <td class="align-middle">{{$perm->name}}</td>
          <td class="align-middle">{{ $perm->label }}</td>
          <td class="text-right align-middle">
            <label class="checkbox-label">
              @if ($role->isAllowedTo($perm->name))
                <input type="checkbox" name="ids[]" checked="checked" value="{{$perm->id}}">
              @else
                <input type="checkbox" name="ids[]" value="{{$perm->id}}">
              @endif
    ...
    <div class="form-group w-100">
      ...
      <input value="{{$role->name}}" type="text" class="form-control py-3 px-3" id="name" name="name" placeholder="Role name...">
    </div>
  </div>
